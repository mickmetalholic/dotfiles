...
class SessionManager {
    constructor(requiredEditorServicesVersion, log, documentSelector) {
    ...
    start() {
        ...
        if (os.platform() === "darwin" && this.getPowerShellVersionLabel() === "alpha") {
            if (!(utils.checkIfFileExists("/usr/local/opt/openssl/lib/libcrypto.1.0.0.dylib") &&
                ...
                const thenable = vscode.window.showWarningMessage("The PowerShell extension will not work without OpenSSL on macOS and OS X when using " +
                    "PowerShell Core alpha", "Show Documentation");
                thenable.then((s) => {
